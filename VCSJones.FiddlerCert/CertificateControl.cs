...
namespace VCSJones.FiddlerCert
{
    ...
    {
        private readonly X509ChainElement _chainElement;

        public CertificateControl(X509ChainElement chainElement) : this()
        {
        ...
        private void CertificateControl_Load(object sender, EventArgs e)
        {
            var certificate = _chainElement.Certificate;
            ...
            if (_chainElement.ChainElementStatus.Length == 0 || _chainElement.ChainElementStatus.All(status => status.Status == X509ChainStatusFlags.NoError))
            {
            ...
            else
            {
                ...
                certStatusToolTip.SetToolTip(certStatusImage, $"Errors: {string.Join(", ", _chainElement.ChainElementStatus.Select(status => status.Status))}");
            }
